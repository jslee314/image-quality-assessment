...
def compute_features(img):
    ...
    for itr_scale in range(scalenum):
        ...
        # Todo(Step 1) Extract Natural Scene Statistics (NSS) - 자연 장면 통계 추출
        print('step 1')
        # calculating MSCN coefficients

        # 1) mu: Subtract local mean field
        mu = cv2.GaussianBlur(im, (7, 7), 1.166)
        mu_sq = mu * mu

        # 2) sigma: Divide by local variance field
        sigma = cv2.GaussianBlur(im * im, (7, 7), 1.166)
        sigma = (sigma - mu_sq) ** 0.5

        # structdis: MSCN(i, j) image
        structdis = im - mu
        ...
        # Todo(Step 2) Calculate Feature Vectors - 특징 벡터 계산
        print('step 2')
...
def test_measure_BRISQUE(imgPath):
    imgPath = 'D:\Data\ImageQuality\image4.jpg'
    print(imgPath)

    # read image from given path
    dis = cv2.imread(imgPath, 1)
    cv2.imshow('Original', dis)
    cv2.waitKey(0)
    ...
    # Todo (Step 3) Prediction of Image Quality Score - 이미지 품질 점수 예측
    print('step3')
